// testpilot.js/src/test/javascript/method-01-namespace.js
...
function Method_Namespace() {
	...
	this.namespaceInGlobalScope = function() {
		$$globalNamespace = method.createNamespace();

		$AssertEquals(method.$method().__namespaces.length, 3, "method has 3 namespaces: " + method.$method().__namespaces.length);
		$AssertNotNull($$globalNamespace, "new namespace is not null");
		$AssertNotNull($$globalNamespace.$method().__getName(), "new namespace name is not null");
		$AssertEquals($$globalNamespace.$method().__getName(), "$$globalNamespace", "new namespace name is correct: " + $$globalNamespace.$method().__getName());
		$AssertTrue($$globalNamespace.$method().__isValid(), "new namespace is in the namespace cache");
		$AssertNotNull($$globalNamespace.$method().__getParent(), "new namespace has parent namespace");
		$AssertEquals($$globalNamespace.$method().__getParent(), rootNamespace, "new namespace parent namespace is the root namespace");
		$AssertEquals($$globalNamespace.$method().__getChildren().length, 0, "new namespace has no child namespaces");

		method.delete($$globalNamespace);

		$Error(function() {

			var obj = $$globalNamespace;
